...
function TrustNetworkVisual({ compact = false }: { compact?: boolean }) {
  const nodes = [
    { x: '50%', y: '18%', icon: Fingerprint, label: 'Score', delay: 0 },
    { x: '20%', y: '54%', icon: Wallet, label: 'Wallet', delay: 0.2 },
    { x: '80%', y: '54%', icon: Coins, label: 'Pool', delay: 0.4 },
    { x: '50%', y: '82%', icon: BadgeCheck, label: 'Passport', delay: 0.6 },
  ];
...
function getLoanStatusLabel(status: Loan['status']) {
  const labels: Record<number, string> = {
    [LoanStatusCode.Pending]: 'Pending',
    [LoanStatusCode.Funded]: 'Funded',
    [LoanStatusCode.Repaid]: 'Repaid',
    [LoanStatusCode.Defaulted]: 'Defaulted',
    [LoanStatusCode.Rejected]: 'Rejected',
  };
    ...
    <div className="glass-card p-4">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <span className={snapshot.enabled && snapshot.isSepolia ? 'badge-success' : 'badge-warning'}>
          {snapshot.enabled && snapshot.isSepolia ? 'On-chain ready' : 'Switch to Sepolia'}
        </span>
      </div>
      {snapshot.enabled && (
    ...
    <div className="app-shell flex h-screen">
      ...
      <motion.aside initial={false} animate={{ x: open ? 0 : typeof window !== 'undefined' && window.innerWidth >= 1024 ? 0 : -280 }} className="fixed lg:static inset-y-0 left-0 z-40 w-64 app-sidebar flex flex-col">
        ...
        <div className="p-4 border-t border-white/10">
          <p className="text-xs text-dark-300 mb-1">Connected</p>
        ...
        <div className="landing-copy">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <div className="inline-flex items-center gap-2 rounded-full border border-primary-300/30 bg-primary-300/10 px-4 py-2 text-sm font-bold text-primary-100 mb-6">
              ...
              Reputation-backed lending on Sepolia
            </div>
            <h1 className="text-5xl md:text-7xl font-black text-white mb-6 leading-tight">
              Trust becomes credit. Repayment becomes reputation.
            </h1>
            <p className="text-lg md:text-xl text-dark-300 max-w-2xl mb-10">
              Request ETH loans, repay through MetaMask, and grow an on-chain credit passport that follows your work history.
            </p>
            <div className="flex flex-col sm:flex-row gap-4">
              <button onClick={handleConnect} className="btn-primary text-lg !px-10 !py-4 inline-flex items-center justify-center gap-2">
                {isConnecting ? 'Connecting...' : isConnected ? 'Open App' : 'Connect Wallet'}
                <ArrowRight className="w-5 h-5" />
              </button>
              <a href="#features" className="btn-ghost">Explore Flow</a>
            </div>
...
function DashboardPage() {
  ...
  const pieData = [{ name: 'Repaid', v: completed.length, c: '#10b981' }, { name: 'Active', v: active.length, c: '#14b8a6' }, { name: 'Pending', v: pending.length, c: '#f0b429' }];
  return (
    <SidebarLayout>
        ...
        <div className="dashboard-hero">
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 rounded-full border border-primary-300/30 bg-primary-300/10 px-3 py-1 text-xs font-bold text-primary-100 mb-4">
              ...
              Reputation passport active
            </div>
            <h1 className="text-3xl md:text-5xl font-black text-white">Welcome, {user?.name || 'Borrower'}</h1>
            <p className="text-dark-300 mt-2 max-w-2xl">Track your trust score, loan lifecycle, and on-chain repayment record from one credit cockpit.</p>
            <div className="mt-5 flex flex-wrap gap-3">
              <Link to="/loans" className="btn-primary text-sm inline-flex items-center gap-2">Request Loan <ArrowRight className="w-4 h-4" /></Link>
              <Link to="/nft" className="btn-ghost !py-3 !text-sm">View Passport</Link>
            </div>
          </div>
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    alert("Loan request submitted on blockchain!");
...
const handleRepay = async (loan: Loan) => {
  ...
  const completed = loans.filter(l => getLoanStatusCode(l.status) === LoanStatusCode.Repaid);
  const loanStats = [
    { l: 'Pending', v: pending.length, className: 'text-warning-400', icon: Activity },
    { l: 'Funded', v: active.length, className: 'text-success-400', icon: Coins },
    { l: 'Repaid', v: completed.length, className: 'text-primary-400', icon: BadgeCheck },
  ];
  return (
    <SidebarLayout>
      <div className="space-y-6">
        <div className="section-heading flex items-center justify-between flex-wrap gap-4">
          <div>
            <p className="text-xs font-black uppercase text-primary-300 tracking-wider">Borrower console</p>
            <h1 className="text-3xl font-black text-white">Loan Flow</h1>
            <p className="text-dark-300">Request, track, and repay ETH loans tied to your reputation passport.</p>
          </div>
        ...
        {loans.length > 0 && (
          <div className="glass-card p-4">
            <h3 className="font-semibold text-dark-100 mb-4">On-chain Loans</h3>
              ...
              <motion.div key={l.id} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="loan-card">
                <div className="flex flex-wrap justify-between items-start gap-3 mb-4">
                  <div>
                    ...
                    <span className="text-2xl font-black text-white">{Number(l.principal).toFixed(4)} ETH</span>
                  </div>
                ...
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-xs text-dark-300">
                  ...
                  <span className="font-mono">
                    Due: {Number(l.remainingDue ?? l.repaymentDue ?? 0).toFixed(4)} ETH
                  </span>
                  ...
                  >
                    Repay <ArrowRight className="w-4 h-4" />
                  </button>
                )}
                </div>
              </motion.div>
...
function PoolPage() {
  ...
  try {
      ...
      setTxMessage(
        `On-chain deposit confirmed: ${hash.slice(0, 10)}...${hash.slice(-6)}`
      );
      ...
      <div className="space-y-8">
        <section className="pool-hero">
          <div className="relative z-10 max-w-2xl">
            <div className="page-eyebrow"><Coins className="w-4 h-4" /> Sepolia Liquidity Market</div>
            <h1 className="page-title">Put idle ETH to work.</h1>
            <p className="page-subtitle">Supply liquidity to community-backed loans and track every deposit directly through your Remix-deployed contract.</p>
            <div className="flex flex-wrap gap-3 mt-7">
              <button onClick={() => setShowDeposit(true)} className="btn-primary flex items-center gap-2">
                <Plus className="w-4 h-4" /> Deposit ETH
              </button>
              <span className={chain?.enabled && chain.isSepolia ? 'chain-pill chain-pill-live' : 'chain-pill'}>
                <span className="chain-dot" />
                {chain?.enabled && chain.isSepolia ? 'Sepolia live' : 'Check network'}
              </span>
            </div>
          </div>
        ...
        {pool && (
          <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
            <StatCard title="Total Liquidity" value={`$${Number(pool.total_liquidity).toLocaleString()}`} icon={Coins} />
            <StatCard title="Available Funds" value={`$${Number(pool.available_funds).toLocaleString()}`} icon={DollarSign} index={1} />
            <StatCard title="Interest Rate" value={`${pool.interest_rate}%`} icon={TrendingUp} index={2} />
            <StatCard title="Utilization" value={`${utilization.toFixed(0)}%`} icon={Activity} index={3} />
          </div>
        )}
        ...
        <div className="grid xl:grid-cols-[1.35fr_0.65fr] gap-6 items-start">
          ...
          <section className="flow-panel">
            <div className="flex items-center justify-between mb-5">
              <div>
                <p className="text-xs font-black uppercase text-primary-600">Liquidity cycle</p>
                <h2 className="text-lg font-bold text-dark-100">How funds move</h2>
              </div>
              <Activity className="w-6 h-6 text-primary-500" />
            </div>
            <div className="pool-steps">
              {[
                { icon: Wallet, title: 'Deposit', text: 'Supply Sepolia ETH' },
                { icon: Landmark, title: 'Fund', text: 'Back trusted borrowers' },
                { icon: TrendingUp, title: 'Earn', text: 'Receive repayment yield' },
              ].map((step, i) => (
        ...
        <AnimatePresence>{showDeposit && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-dark-100/20 z-50 flex items-center justify-center p-4" onClick={() => setShowDeposit(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="glass-card p-6 w-full max-w-sm space-y-4" onClick={e => e.stopPropagation()}>
              ...
              <input placeholder="Amount (Sepolia ETH)" type="number" step="0.001" min="0" value={amount} onChange={e => setAmount(e.target.value)} className="input-field" />
              <div className="grid grid-cols-4 gap-2">{[0.01, 0.05, 0.1, 1].map(v => <button key={v} onClick={() => setAmount(v.toString())} className="glass py-2 text-sm hover:bg-dark-700">{v} ETH</button>)}</div>
              <p className="text-xs text-dark-400">This will send Sepolia ETH to your Remix-deployed TrustLend contract.</p>
...
function NftPage() {
  ...
  const passportId = address ? `${address.slice(0, 6)}...${address.slice(-4)}` : 'Not connected';
  if (loading) return <SidebarLayout><Loader2 className="w-8 h-8 animate-spin text-primary-500 mx-auto mt-32" /></SidebarLayout>;
  return (
    <SidebarLayout>
      <div className="space-y-8">
        <section className="passport-hero">
          <div className="relative z-10 max-w-xl">
            <div className="page-eyebrow"><Fingerprint className="w-4 h-4" /> Verifiable Credit Identity</div>
            <h1 className="page-title">Your reputation, made portable.</h1>
            <p className="page-subtitle">Every completed repayment strengthens a public record of reliability that belongs to your wallet.</p>
            <div className="grid grid-cols-3 gap-3 mt-7">
              <div className="passport-stat"><span>{(user?.successful_loans || 0) * 100}</span><small>POINTS</small></div>
              <div className="passport-stat"><span>{nfts.length}</span><small>BADGES</small></div>
              <div className="passport-stat"><span>LVL {user?.reputation_level || 1}</span><small>STATUS</small></div>
            </div>
          </div>
          <motion.div className="credit-passport" initial={{ opacity: 0, rotateY: -12, y: 16 }} animate={{ opacity: 1, rotateY: 0, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <div className="relative z-10 mt-auto">
              <p className="text-xs uppercase font-black text-primary-200">TrustLend Credit Passport</p>
              ...
              <div className="flex items-end justify-between gap-4 mt-5">
                <div><small className="text-dark-400">WALLET ID</small><p className="font-mono text-sm text-white">{passportId}</p></div>
        ...
        <div className="grid xl:grid-cols-[0.72fr_1.28fr] gap-6 items-start">
          <section className="glass-card p-6">
            <div className="flex items-center justify-between gap-4 mb-5">
              <div><p className="text-xs font-black uppercase text-primary-600">Next milestone</p><h2 className="text-xl font-bold text-dark-100">Tier Progress</h2></div>
            ...
            <ProgressBar value={progress.current} max={progress.target} label="Successful repayments" color="accent" />
            <p className="mt-4 text-sm text-dark-400">Complete funded loans on Sepolia to unlock the next passport mark.</p>
          </section>
          <OnChainStatus snapshot={chain} />
        </div>
        ...
        <section>
          <div className="flex items-end justify-between gap-4 mb-4">
            <div><p className="text-xs font-black uppercase text-primary-400">Collection</p><h2 className="text-2xl font-bold text-white">Achievement Badges</h2></div>
          ...
          {nfts.length > 0 ? (
            <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">{nfts.map((n, i) => (
              <motion.div key={n.id} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="achievement-badge" style={{ '--badge-color': tiers[n.tier]?.color || '#fff' } as React.CSSProperties}>
                ...
                <p className="text-dark-400 text-xs uppercase font-black mt-1">{n.tier} tier</p>
              </motion.div>
            ))}</div>
          ) : <div className="empty-passport"><Lock className="w-10 h-10" /><div><p className="text-white font-bold">Your first badge is waiting</p><p className="text-dark-400 text-sm">Repay a funded loan to begin the collection.</p></div></div>}
        </section>

        <section className="tier-track">
          <div><p className="text-xs font-black uppercase text-primary-600">Passport levels</p><h2 className="text-xl font-bold text-dark-100">Reputation Roadmap</h2></div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-5">{(Object.entries(tiers) as [keyof typeof tiers, typeof tiers.bronze][]).map(([t, c]) => {
            ...
            return <div key={t} className={`tier-step ${earned ? 'tier-step-earned' : ''}`} style={{ '--tier-color': c.color } as React.CSSProperties}>
              <div className="flex items-center gap-2">{earned ? <Unlock className="w-4 h-4" /> : <Lock className="w-4 h-4" />}<span className="font-bold capitalize">{t}</span></div>
              <p className="text-dark-400 text-xs mt-1">{c.loans}+ successful loans</p>
            </div>;
          })}</div>
        </section>
      </div>
      ...
      <div className="space-y-8">
        <section className="profile-hero">
          ...
          <div className="min-w-0 flex-1">
            <div className="page-eyebrow"><BadgeCheck className="w-4 h-4" /> Verified Wallet Profile</div>
            <h1 className="text-3xl md:text-4xl font-black text-white truncate">{form.name || 'Complete your identity'}</h1>
            <p className="text-dark-400 capitalize mt-1">{form.user_type.replace('_', ' ')} profile · Reputation level {user?.reputation_level || 1}</p>
          </div>
        ...
        <div className="grid xl:grid-cols-[360px_1fr] gap-6 items-start">
          <div className="glass-card p-6 xl:sticky xl:top-0">
            <h3 className="font-semibold text-dark-100 mb-4 flex items-center gap-2"><Award className="w-4 h-4 text-primary-500" />Credit Snapshot</h3>
            <TrustGauge score={user?.trust_score || 0} size={140} />
            <div className="profile-facts">
              <div><span>Risk category</span><strong className={`${user?.risk_category === 'low' ? 'text-success-600' : 'text-warning-600'} capitalize`}>{user?.risk_category || 'Unrated'}</strong></div>
              <div><span>Borrowing power</span><strong>${user?.max_loan_amount?.toLocaleString() || 0}</strong></div>
              <div><span>Suggested rate</span><strong>{user?.suggested_interest_rate || 0}%</strong></div>
            </div>
            <div className="score-tip">
              <Sparkles className="w-5 h-5 text-accent-500 shrink-0" />
              <p>Adding verifiable work and education details can strengthen your score.</p>
            </div>
          </div>
          <form onSubmit={handleSubmit} className="glass-card p-5 md:p-8 space-y-6">
            <div className="flex items-center justify-between gap-4">
              <div><p className="text-xs font-black uppercase text-primary-600">Identity data</p><h2 className="text-xl font-bold text-dark-100">Build your credit profile</h2></div>
            ...
            <div className="profile-form-section">
              <p className="text-xs text-primary-700 font-bold uppercase mb-3">Select Your Role</p>
              <div className="grid grid-cols-3 gap-2 mb-4">
                {(['student', 'freelancer', 'gig_worker'] as UserType[]).map(t => (
                  <button key={t} type="button" onClick={() => setForm(f => ({ ...f, user_type: t }))} className={`role-option ${form.user_type === t ? 'role-option-active' : ''}`}>{t.replace('_', ' ')}</button>
                ))}
              </div>
